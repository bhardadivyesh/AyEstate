...
// Ensure the paymentDetail directory exists
const uploadDir = path.join(__dirname, 'paymentDetail');
...
router.post("/post-membershipPriceDetail", upload.fields([
  ...
  try {
    ...
    const membershipPriceDetail = new MembershipPriceDetail({
    ...
    await membershipPriceDetail.save();
    res.status(201).send("Membership added successfully");
  } catch (error) {
...
router.get("/get-membershipPriceDetail", async (req, res) => {
  try {
    const getMembershipDetail = await MembershipPriceDetail.find();
